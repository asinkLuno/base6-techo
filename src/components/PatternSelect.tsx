...
import { PATTERN_GROUPS, patternNameKeys } from "../lib/schema";
    ...
    <FormControl size="small" fullWidth>
      ...
      <Select label={label} value={value} onChange={(e) => onChange(e.target.value as PatternKind)}>
        {PATTERN_GROUPS.map((group) => [
          <ListSubheader key={group.label} disableSticky>{t(`patternGroup.${group.label}`)}</ListSubheader>,
          ...("kinds" in group
            ? group.kinds.map((kind) => <MenuItem key={kind} value={kind}>{t(`pattern.${patternNameKeys[kind]}`)}</MenuItem>)
            : group.subgroups.flatMap((sub) => [
                <ListSubheader key={sub.label} disableSticky sx={{ pl: 3, fontSize: "0.75rem", lineHeight: "28px" }}>
                  {t(`patternGroup.${sub.label}`)}
                </ListSubheader>,
                ...sub.kinds.map((kind) => <MenuItem key={kind} value={kind}>{t(`pattern.${patternNameKeys[kind]}`)}</MenuItem>),
              ])),
        ])}
      </Select>
    </FormControl>
